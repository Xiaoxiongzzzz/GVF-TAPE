...
from diffusion_model.VideoGenerator import VideoGenerator, prepare_video_generator
...
def run_task(task_name, suite_name, cfg, checkpoint_path, render_device, rollout_times, rollout_horizon, is_replace_checkpoint_keys):
    ...
    video_generator = prepare_video_generator(unet_path="/mnt/home/ZhangXiaoxiong/Documents/AVDC/results/RFlow/model_33000.pt", device=device)
    ...
    for time in tqdm(range(rollout_times), desc=f"Task: {task_name}"):
        ...
        for step in range(rollout_horizon):
            ...
            diffusion_image = diffusion_image.squeeze(1)
            feature = video_generator(task_prompt, diffusion_image)
            feature = feature.unsqueeze(1)
            feature = rearrange(feature, "b t (c f) h w -> b t c f h w", c=3)
        ...
        output_dir = f"./results/video_policy/{task_name}"
