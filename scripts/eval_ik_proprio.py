...
class IKEvaluator:
    ...
    def _setup_experiment(self):
        ...
        base_dir = self.config.get("output", {}).get(
        ...
        self.base_output_dir = os.path.join(
            base_dir, f"{exp_name}_{timestamp}_{seed_suffix}"
        )
        ...
        shutil.copy2(
            self.config_path, os.path.join(self.base_output_dir, config_filename)
        )
    ...
    def _setup_tasks(self):
        ...
        if self.config["task"]["selected_tasks"]:
            ...
            self.selected_tasks = {
                f"task{i+1}": task_name for i, task_name in enumerate(available_tasks)
            }
    ...
    def _get_proprio_state(self, obs):
        ...
        try:
            ...
            if isinstance(gripper_pos, np.ndarray):
                ...
                gripper_value = np.array([float(gripper_pos)]).reshape(
                    1,
                )

            return np.concatenate(
                [
                    obs["robot0_eef_pos"].reshape(
                        3,
                    ),
                    obs["robot0_eef_quat"].reshape(
                        4,
                    ),
                    gripper_value,
                ],
    ...
    def _compute_pid_control(self, goal_out, proprio_st, prev_error, integral_error):
        ...
        control_quat = (
            st.Rotation.from_quat(goal_out[3:7])
        ...
        gripper_goal = (
            goal_out[7] if goal_out[7] > self.config["pid"]["grip_threshold"] else 0.00
        )
        ...
        control_signal = np.concatenate(
            [
                control_signal_trans.reshape(-1),
    ...
    def _generate_video(self, task_embedding, side_view):
        ...
        with torch.cuda.amp.autocast():  # Optional: Use mixed precision
            video_clip = rearrange(
                self.video_generator(task_embedding, side_view),
                "b (f c) h w -> (b f) c h w",
                c=self.channel_num,
            )
            ...
            # Process RGB
            video_clip_np = (
                video_clip[:, :3, :, :].permute(0, 2, 3, 1).detach().cpu().numpy() * 255
            ).astype(np.uint8)

            # Process depth if needed
            if self.use_depth:
                video_clip_depth = (
                    video_clip[:, 3:, :, :].permute(0, 2, 3, 1).detach().cpu().numpy()
                    * 255
                ).astype(np.uint8)
            else:
    ...
    def _predict_goal(self, goal_img):
        ...
        with torch.cuda.amp.autocast():  # Optional: Use mixed precision
            if self.depth_transform is not None and self.use_depth:
                ...
                goal_img_depth = self.depth_transform(
                    goal_img[3:, :, :].to(self.device)
                ).unsqueeze(0)
                goal_img_rgb = self.transform(
                    goal_img[:3, :, :].to(self.device)
                ).unsqueeze(0)
                ...
                goal_img = self.transform(goal_img[:3, :, :].to(self.device)).unsqueeze(
                    0
                )
    ...
    def _run_single_test(self, task_name, test_time, task_output_dir, task_embedding):
        ...
        for video_sample in range(self.config["num_video_samples"]):
            ...
            # Generate video
            video_clip, video_clip_np, video_gen_time, video_clip_depth = (
                self._generate_video(task_embedding, side_view)
            )
    ...
    def _execute_video_actions(
        self, env, obs, video_clip, video_clip_np, video_clip_depth, horizon
    ):
    ...
    def _run_pid_control(self, env, obs, goal_out, goal_img_np, goal_img_depth=None):
        ...
        while (
            np.linalg.norm(proprio_st - goal_out)
            > self.config["pid"]["convergence_threshold"]
            and pid_step < self.config["pid_max_steps"]
        ):
            pid_step += 1
            ...
            frame = np.concatenate(
                [cv2.flip(obs["agentview_image"], 0), goal_img_np], axis=1
            )
            ...
            if goal_img_depth is not None:
                goal_depth_colored = cv2.applyColorMap(
                    goal_img_depth.squeeze(), cv2.COLORMAP_VIRIDIS
                )
    ...
    def _save_test_results(self, task_output_dir, test_time, status, roll_out_video):
        ...
        with imageio.get_writer(
            video_path, mode="I", duration=self.config["video"]["duration"]
        ) as writer:
        ...
        with open(info_path, "a") as f:
            f.write(
                f"Test {test_time}: {'Success' if status == 'success' else 'Fail'}\n"
            )
    ...
    def evaluate_task(self, task_id, task_name):
        ...
        with h5py.File(
            f"./cache/{self.config['task']['suite_name']}_embedding.hdf5"
        ) as f:
    ...
    def _save_task_summary(
        self, task_output_dir, task_name, success_rate, successes, inference_times
    ):
        ...
        with open(os.path.join(task_output_dir, "task_results.txt"), "w") as f:
            ...
            f.write(
                f"Successful attempts: {successes}/{self.config['num_test_pr_task']}\n"
            )
            f.write(
                f"kp_trans={self.config['pid']['kp_trans']}, ki_trans={self.config['pid']['ki_trans']}, "
                f"kd_trans={self.config['pid']['kd_trans']}\n"
            )
            f.write(
                f"kp_rot={self.config['pid']['kp_rot']}, ki_rot={self.config['pid']['ki_rot']}, "
                f"kd_rot={self.config['pid']['kd_rot']}\n"
            )
            f.write(
                f"kp_grip={self.config['pid']['kp_grip']}, ki_grip={self.config['pid']['ki_grip']}, "
                f"kd_grip={self.config['pid']['kd_grip']}\n"
            )
            f.write("\nInference Times:\n")
            f.write(
                f"Video Generator - Avg: {np.mean(inference_times['video_gen'])*1000:.2f}ms, "
                f"Std: {np.std(inference_times['video_gen'])*1000:.2f}ms\n"
            )
            f.write(
                f"IK Model - Avg: {np.mean(inference_times['ik_model'])*1000:.2f}ms, "
    ...
    def _evaluate_task_wrapper(self, task_id, task_name):
        ...
        try:
            ...
            for test_time in range(self.config["num_test_pr_task"]):
                print(
                    f"Process {os.getpid()}: {task_id} test {test_time+1}/{self.config['num_test_pr_task']}"
                )
            ...
            print(
                f"Process {os.getpid()} encountered error while evaluating task {task_id}: {e}"
            )
            import traceback
    ...
    def _save_overall_results(self, all_task_results, inference_times):
        ...
        with open(os.path.join(self.base_output_dir, "overall_results.txt"), "w") as f:
            ...
            f.write(
                f"Overall success rate across all tasks: {overall_success_rate:.2f}%\n\n"
            )

            # Calculate and write overall inference times
            all_video_gen_times = [
                time
                for task_times in inference_times.values()
            ...
            f.write(
                f"Video Generator - Avg: {np.mean(all_video_gen_times)*1000:.2f}ms, "
                f"Std: {np.std(all_video_gen_times)*1000:.2f}ms\n"
            )
            f.write(
                f"IK Model - Avg: {np.mean(all_ik_times)*1000:.2f}ms, "
