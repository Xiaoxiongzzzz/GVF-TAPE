...
class FeatureExtractor(nn.Module):
    def __init__(
        self,
        unet: nn.Module,  # the UnetBridge class
        tokenizer,
        text_encoder,
        device,
    ...
    def extract_feature(self, x, t, text_embedding):
        ...
        feature = [
            F.interpolate(
                f, size=(6, *space_size), mode="trilinear", align_corners=True
            )
            for f in feature
        ]
    ...
    def extract_from_list(self, xs: list, task_prompt):
        ...
        """
        xs = [
            self.resize_diffusion_image(
                torch.from_numpy(x).to(self.device).permute(2, 0, 1)
            ).unsqueeze(0)
            for x in xs
        ]  # (b, c, h, w)
        x_cond = xs[0]
