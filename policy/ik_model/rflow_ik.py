...
class FlowIKModel(nn.Module):
    def __init__(self, resnet, device, output_dim=7, sub_task_dim=12, hidden_dim=256, loss_type="L2"):
        """
        Input:
            resnet: nn.module
        ...
        assert loss_type in ["L1", "L2"], "Invalid loss type, only support L1 and L2 loss"
    ...
    def train_loss(self, x, cond, regularization_term=None):
        ...
        if self.loss_type == "L1":
            loss_dict["velocity_loss"] = torch.mean(torch.abs(pred_velocity - (x - noise)))

        if regularization_term is not None:
            loss_dict["regularization_loss"] = torch.mean((self.regularization_head(cond) - regularization_term)**2)
