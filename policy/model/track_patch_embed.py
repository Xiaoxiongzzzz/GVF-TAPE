...
class TrackPatchEmbed(nn.Module):
    def __init__(self, num_track_ts, num_track_ids, patch_size, in_dim, embed_dim):
        super().__init__()
        ...
        assert (
            self.num_track_ts % self.patch_size == 0
        ), "num_track_ts must be divisible by patch_size"
        ...
        self.conv = nn.Conv1d(
            in_dim, embed_dim, kernel_size=patch_size, stride=patch_size, bias=True
        )
