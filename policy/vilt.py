...
class BCViLTPolicy(nn.Module):
    ...
    def _setup_feature_encoder(self, network_name, patch_size, channel_list, no_patch_embed_bias,
                                ):
    ...
    def _setup_spatial_positional_embeddings(self):
        ...
        modality_idx += [modality_idx[-1] + 1] * self.feature_num_patches   #for feature modality embeding
    ...
    def _setup_policy_head(self, network_name, **policy_head_kwargs):
        ...
        if self.late_fusion:
            ...
            feature_pos_embed_late_fusion = nn.Parameter(torch.randn(1, self.feature_num_patches, self.spatial_embed_size))
            self.register_parameter("feature_token", feature_token)
            self.register_parameter("feature_pos_embed_late_fusion",feature_pos_embed_late_fusion)
        else:
    ...
    def spatial_encode(self, obs, feature, extra_states, return_feature=False, debug=False):
        """
        ...
        img_feature_encoded = torch.cat([img_encoded, feature_encoded], -2)  # (b, t, 2*num_img_patch + num_feature_patch, c)
        ...
        encoded = torch.cat([spatial_token, img_feature_encoded], -2)  # (b, t, 2*num_img_patch + num_feature_patch + 2, c)
    ...
    def forward(self, obs, feature, extra_states):
        ...
        x, feature_encoded = self.spatial_encode(obs, feature, extra_states, return_feature=True)  # x: (b, t, 2+num_extra, c)  featrue_encoded: (b, t, patches, c)
    ...
    def act(self, obs, feature, extra_states):
        ...
        with torch.no_grad():
            latent, encoded_feature = self.spatial_encode(obs, feature, extra_states, return_feature=True)
            ...
            action_token = self.temporal_encode(temporal_input)[:,-1]       #The latest action token
            if self.late_fusion:
                feature_token = self.spatial_encode_feature(encoded_feature)[:,0]      #b c
                action_token = torch.cat([action_token, feature_token], dim=-1)  # (b, channel)
    ...
    def reset(self, device):
        ...
        self.latent_buffer = [torch.zeros(1, 1, 4, self.temporal_embed_size).to(device)\
