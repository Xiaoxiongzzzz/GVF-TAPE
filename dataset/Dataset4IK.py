...
class LiberoSuiteDataset4IK(Dataset):
    def __init__(
        self, suite_path, ratio, extra_state_keys=None, augmentation=True, mode="train"
    ):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(os.path.join(self.suite_path, hdf5_name), "r") as f:
            ...
            side_view_goal = (
                torch.flip(
                    torch.from_numpy(demo["obs"]["agentview_rgb"][time_index]),
                    dims=[-3],
                ).to(torch.float32)
                / 255.0
            ...
            side_view_current = (
                torch.flip(
                    torch.from_numpy(demo["obs"]["agentview_rgb"][current_index]),
                    dims=[-3],
                ).to(torch.float32)
                / 255.0
            )  # height, width, channel
            eye_in_hand_current = (
                torch.flip(
                    torch.from_numpy(demo["obs"]["eye_in_hand_rgb"][current_index]),
                    dims=[-3],
                ).to(torch.float32)
                / 255.0
            )
            observation_current = {
                k: torch.from_numpy(demo["obs"][k][current_index]).to(torch.float32)
                for k in self.extra_state_keys
            }  # {k:(n)}
            observation_goal = {
                k: torch.from_numpy(demo["obs"][k][time_index]).to(torch.float32)
                for k in self.extra_state_keys
            }  # {k:(n)}
            action_current = torch.from_numpy(demo["actions"][current_index]).to(
                torch.float32
            )

        visual_obs = torch.stack(
            [side_view_current, eye_in_hand_current], dim=0
        ).permute(
            0, 3, 1, 2
        )  # (views, channel, height, width)
        ...
        observation_goal["ee_states"] = (
            torch.cat(
                [
                    observation_goal["ee_states"][:3],
                    torch.from_numpy(
                        R.from_euler(
                            "xzy", observation_goal["ee_states"][3:], degrees=False
                        ).as_quat()
                    ),
