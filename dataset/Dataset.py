...
class LiberoDataset(Dataset):
    def __init__(
        ...
        print(
            f"The number of demos is:{len(self.demos)}, the number of data is: {self.length}"
        )
            ...
            self.augmentator = transforms.Compose(
                [
                    ...
                    transforms.RandomCrop(
                        size=(128, 128), padding=4, fill=0, padding_mode="constant"
                    ),
                ]
            )
            ...
            if time_index >= self.num_frame_stack:
                agentview = torch.from_numpy(
                    demo["obs"]["agentview_rgb"][
                        time_index - self.num_frame_stack + 1 : time_index + 1
                    ]
                )  # (num_frame_stack, height, width, channel)
                ...
                visual_obs = torch.stack(
                    [agentview, eye_in_hand], dim=0
                )  # (views, num_frame_stack, height, width, channel)
                extra_state = {
                    k: torch.from_numpy(
                        demo["obs"][k][
                            time_index - self.num_frame_stack + 1 : time_index + 1
                ...
                agentview = torch.from_numpy(
                    demo["obs"]["agentview_rgb"][: time_index + 1]
                )
                eye_in_hand = torch.from_numpy(
                    demo["obs"]["eye_in_hand_rgb"][: time_index + 1]
                )
                visual_obs = torch.stack([agentview, eye_in_hand], dim=0)
                extra_state = {
                    k: torch.from_numpy(demo["obs"][k][: time_index + 1])
                    for k in self.extra_state_keys
                }
                action = torch.from_numpy(demo["actions"][: time_index + 1])
                visual_obs, extra_state, action = self.pad_to_same_length(
                    visual_obs, extra_state, action
                )  # pad time to num frame stack with 0

        extra_state = {k: v.to(torch.float32) for k, v in extra_state.items()}
        action = action.to(torch.float32)

        visual_obs = torch.flip(
            visual_obs, dims=[-3]
        )  # Vertical Flip the image (views, num_frame_stack, height, width, channel)
        visual_obs = visual_obs.permute(
            0, 1, 4, 2, 3
        )  # (views, num_frame_stack, channel, height, width)
...
class LiberoSuiteDataset(Dataset):
    def __init__(
        ...
        self.resize_diffusion_image = transforms.Resize(
            self.diffusion_image_size, antialias=True
        )
        if self.augmentation:
            self.augmentator = transforms.Compose(
                [
        ...
        print(
            f"The number of demos is {sum([len(demos) for demos in self.train_demos])}"
        )
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(os.path.join(self.suite_path, hdf5_name), "r") as f:
            ...
            if time_index >= self.num_frame_stack:
                ...
                if self.return_feature:
                    feature = torch.from_numpy(
                        demo["feature"][: time_index + 1]
                    )  # time, channel, height, width
                    visual_obs, extra_state, action, feature = self.pad_to_same_length(
                        visual_obs, extra_state, action, feature
                    )  # pad time to num frame stack with 0
                else:
                    visual_obs, extra_state, action, _ = self.pad_to_same_length(
                        visual_obs, extra_state, action
                    )  # pad time to num frame stack with 0
    ...
    def get_index_codebook(self):
        ...
        for i, hdf5_name in enumerate(self.hdf5_files):
            for j, demo in enumerate(self.train_demos[i]):
                index_codebook.extend(
                    [hdf5_name, demo, t] for t in range(self.demos_length[i][j])
                )
