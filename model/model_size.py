...
def compare_models(image_size=224, out_size=8):
    ...
    resnet50_backbone = sum(
        p.numel() for p in resnet.resnet50.parameters() if p.requires_grad
    )
    resnet50_mlp = sum(p.numel() for p in resnet.mlp.parameters() if p.requires_grad)

    resnet18_backbone = sum(
        p.numel() for p in resnet18.resnet18.parameters() if p.requires_grad
    )
